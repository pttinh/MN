...
const int QUATER_BLOCKCOUNT = 12960;	// 90 * 1440
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(nHeight < QUATER_BLOCKCOUNT)
        nRewardCoinYear = 5 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (2 * QUATER_BLOCKCOUNT))
        nRewardCoinYear = 4 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (3 * QUATER_BLOCKCOUNT))
        nRewardCoinYear = 3 * 4 * MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (4 * QUATER_BLOCKCOUNT))
        nRewardCoinYear = 2 * 4* MAX_MINT_PROOF_OF_STAKE;
    else if(nHeight < (5 * QUATER_BLOCKCOUNT))
        nRewardCoinYear = 1 * 4* MAX_MINT_PROOF_OF_STAKE;

    int64 nSubsidy = nCoinAge / COIN * nRewardCoinYear / 365;
...
static const int64 nTargetSpacingWorkMax = 3 * nWorkTargetSpacing;
...
unsigned int GetNextTargetRequired_Normal(const CBlockIndex* pindexLast, bool fProofOfStake)
{
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if(pindexLast->nHeight + 1 > change)
        return GetNextTargetRequired_Normal(pindexLast, fProofOfStake);
    else
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MoneyCoin
    bool fStrictPayToScriptHash = true; // Always active in MoneyCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MoneyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Who don't need money? Money for all.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1399999999;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 13923120;
        ...
        assert(block.hashMerkleRoot == uint256("0xa853c8c88d8d210032b1426d13517d2e9636982a6633640e86a6b9e612f4d104"));
